        ...
        while num_syllables < M:
            ...
            for i in range(self.D):
                if num_syllables + self.syllable_dict[i][1][0] > M:
                    emission_p[i] = 0
                # Added meter logic here (I hope it's correct)
                elif 10 - num_syllables - self.syllable_dict[i][1][0] % 2 == 0 \
                     and i not in self.starts_unstressed_set:
                    emission_p[i] = 0
                elif 10 - num_syllables - self.syllable_dict[i][1][0] % 2 == 1 \
                     and i not in self.starts_stressed_set:
                    emission_p[i] = 0

            if np.sum(emission_p) == 0:
                print ("No possible outcomes from this state...fishy")
                continue
